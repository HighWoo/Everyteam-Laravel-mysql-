...
@if($classcount==0)
@else
  @if($classval->currentPage()==1)
  <a class="buttonpn previous" href=""onclick="alert('첫 페이지 입니다')"><-이전</a>
  @else
  <a class="buttonpn previous" href="{{$classval->previousPageUrl()}}"><-이전</a>
  @endif
  @if($classval->currentPage()==$classval->lastPage())
  <a class="buttonpn " href=""onclick="alert('마지막 페이지 입니다')">다음-></a>
  @else
  <a class="buttonpn " href="{{$classval->nextPageUrl()}}">다음-></a>
  @endif
@endif
</div>
</div>
